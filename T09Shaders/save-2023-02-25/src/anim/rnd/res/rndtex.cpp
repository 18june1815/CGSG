...
int res::AddImg( std::string Name, int W, int H, int C, BYTE *Bits)
{
  int mips;
...
int res::TexAddFmt( std::string Name, int W, int H, int GLType)
{
  if (NumOfTextures > MAX_TEXTURES)
    return -1;
...
void res::AddTexture( material *Mtl, std::string TexName, const char *TexFile )
{
  HBITMAP hBm;  
  static int TexNo = -1;
...
int res::AddTextureFromFile( material *Mtl, std::string TexName, const char *FileName)
{
  int res = -1;
  HBITMAP hBm;  
...
int res::FindTexture( std::string name )
{
  int TexNo = -1;
